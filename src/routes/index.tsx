...
function Index() {
  ...
  const [labCommand, setLabCommand] = useState<{ prompt: string; seq: number } | undefined>();
  ...
  const send = useCallback(
    async (text: string) => {
          ...
          onEvent(ev) {
            ...
            try {
              ...
              if (delta) {
                ...
                // Handle Action Tags
                if (full.includes("[[ACT:OPEN_LAB]]")) setShowLab(true);
                if (full.includes("[[ACT:CLOSE_LAB]]")) setShowLab(false);
                if (full.includes("[[ACT:REBUILD|")) {
                  const match = full.match(/\[\[ACT:REBUILD\|(.*?)\]\]/);
                  if (match?.[1]) {
                    setShowLab(true);
                    setLabCommand({ prompt: match[1], seq: Date.now() });
                  }
                }
              }
            } catch { /* ignore */ }
      ...
      setMessages((m) => [...m, { role: "assistant", content: full.replace(/\[\[ACT:.*?\]\]/g, "") }]);
      setStreaming("");
      if (full.trim()) await speak(full.replace(/\[\[ACT:.*?\]\]/g, ""));
        ...
        <section className="my-6 flex flex-1 flex-col items-center justify-center gap-6">
          {showLab ? (
            <div className="w-full max-w-2xl aspect-square md:aspect-video">
              <EngineeringLab command={labCommand} />
            </div>
          ) : (
          ...
          <ul className="space-y-3">
            ...
            {streaming && (
              <li className="text-sm">
                ...
                <span className="text-foreground/90">{streaming.replace(/\[\[ACT:.*?\]\]/g, "")}</span>
              </li>
            )}
          </ul>
        </section>
